...
{
  /* Instrumentation */
  if (__glibc_ioinstr_hooks != NULL && __glib_ioinstr_entered == false && __glibc_ioinstr_hooks->pwrite64 != NULL) {
    __glib_ioinstr_entered = true;
    int ret = __glibc_ioinstr_hooks->pwrite64(fd, buf, count, offset);

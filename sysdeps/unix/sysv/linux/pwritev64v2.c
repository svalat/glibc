...
{
  /* Instrumentation */
  if (__glibc_ioinstr_hooks != NULL && __glib_ioinstr_entered == false && __glibc_ioinstr_hooks->pwritev64v2 != NULL) {
    __glib_ioinstr_entered = true;
    int ret = __glibc_ioinstr_hooks->pwritev64v2(fd, vector, count, offset, flags);
